    ...
    for(var i = 0; i < parsed.length; i++) {
        ...
        if(isInflect(parsed[i])) { // 활용형 여부
            ...
            if(isSame(parsed[i], replace.hae.Inflect)) { // 활용형 자체가 사전에 정의되었나
                ...
                // # TODO
                // First if is never true
                // Weird splice; looks like attemp to delete bricks[b]
                // Not wise to change len of array inside for loop; require temp array
                for(var b = 0; b < bricks.length; b++) {
                    if(bricks[b].toString() === si.toString())
            ...
            if(flag) { // 교체가 일어났을 때
                if(bricks) { // 활용형을 분해했을 때
                    ...
                    else if (bricks.length === 1 && shorten[normalizeMorphemes(bricks.unshift(parsed[i-1]))]) { // 앞 형태소를 합쳐서 보기 // TODO: also never true; unshift return int
                        if (shorten[ normalizeMorphemes([parsed[i-1], newmor ]) ] ) { // 앞 형태소를 합쳐서 보기 // TODO: newmor is undefined
        ...
        //// 활용형이 아님
        if(parsed[i].toString() === si.toString()) { // 선어말어미 -시- 제거 // TODO: never true
